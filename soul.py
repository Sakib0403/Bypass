...
TOKEN = '8072019673:AAG0-xuLAPNpUjvhtF1r0z6aKoOGDPn32eM'
MONGO_URI = 'mongodb+srv://AlbertHacks:9161AlbertxSakibOp@albertrana.yvjav.mongodb.net'
FORWARD_CHANNEL_ID = -1002484104838
CHANNEL_ID = -1002484104838
error_channel_id = -1002484104838

logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)

client = MongoClient(MONGO_URI, tlsCAFile=certifi.where())
db = client['albertrana']
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 10099:
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢👉@OfficialAlbertRana 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨.*", parse_mode='Markdown')
            return
